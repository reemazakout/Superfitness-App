...
const Step5Form = ({ onSubmit, onBack, defaultValues }: Step5FormProps) => {
  ...
  // Find the index of the selected height in the visible array
  const getSelectedIndex = () => {
    const visibleNumbers = getVisibleNumbers();
    return visibleNumbers.findIndex((height) => height === selectedHeight);
  };

  const handleNumberClick = (height: number) => {
  ...
  const getNumberStyle = (height: number, isSelected: boolean) => {
    ...
    const currentIndex = visibleNumbers.findIndex((n) => n === height);
              ...
              >
                ...
                >
                  {visibleNumbers.map((height) => (
                    <div
                      key={height}
                      onClick={() => handleNumberClick(height)}
                      style={{
                        ...getNumberStyle(height, height === selectedHeight),
                        lineHeight: "1",
                        display: "flex",
                        alignItems: "center",
